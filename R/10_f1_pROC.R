...
PartialROC <- function(Occur=NA, ModelFile=NA, OmissionVal=NA, RandomPercent=NA, NoOfIteration=NA)
{

  InRast = raster(ModelFile)
...
#  Occur = read.table(TestFile, header=T, sep =",")
#  Occur = Occur[,c("x","y")]
...
AreaRow <- lapply(X = 1:NoOfIteration,FUN =  function(x){
    ...
    df.proc <- data.frame(t(sapply(AreaRow,c)))
    rocparcial <- mean(df.proc$AUC_ratio)
    p <- length(which(df.proc$AUC_ratio >1))/500
    p_valor <- 1-p
    results <- cbind(ModelFile,rocparcial,p_valor)
  return(results)
