# Por: Carlos Muñoz-22 de agosto de 2017
...
selection <- function(sp, path.eval="output/10_eval_all/",
  ...
  # extraer el AICc y estandarizarlo de 0 a 1 (entre mas pequeño mejor)
  AICc_ <- (eval[, "AICc"] - (min(eval[, "AICc"]))) / (max(eval[, "AICc"]) - min(eval[, "AICc"]))
  # extraer la tasa de omision TTP y estandarizarlo de 0 a 1 (entre mas pequeño mejor)
  OR_cal <- (eval[, "TTP_OR_cal"] - (min(eval[, "TTP_OR_cal"]))) / (max(eval[, "TTP_OR_cal"] - min(eval[, "TTP_OR_cal"])))
  # extraer el area predicha y estandarizarla de 0 (mas grande) a 1(mas pequeño) (entre mas pequeño mejor)
